...
import userSalute from "./_helpers/formatSalute";
...
const SideMenu = () => {
  ...
  return (
    <>
      ...
      {data?.user ? (
        <div className="flex items-center justify-between px-3 py-6 border-b border-solid border-secondary">
          <div className="flex items-center gap-3">
            <Avatar>
              <AvatarImage src={data.user?.image ?? ""} />
            </Avatar>
            <div className="flex flex-col">
              <h2 className="font-semibold truncate overflow-y-hidden max-w-40">{`${data.user.name?.split(" ")[0]}, ${userSalute()}`}</h2>
              <p className="text-zinc-400 text-sm truncate overflow-y-hidden max-w-44">{data.user.email}</p>
            </div>
          </div>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="secondary" size="icon" className="relative right-3text-sm z-20">
                <LogOutIcon size={18} />
              </Button>
            </AlertDialogTrigger>
        ...
        <div className="flex flex-col gap-3 px-3 py-6 border-b border-solid border-secondary">
          <div className="flex items-center gap-2">
            <UserIcon size={32} />
            <h2 className="font-semibold">Visitante. {userSalute()}</h2>
          </div>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent className="flex flex-col items-center w-[90%] sm:w-auto h-auto sm:max-w-md overflow-y-auto">
              ...
              <AlertDialogFooter className="mt-2 flex gap-2">
                <AlertDialogAction
                  className="min-w-[134px] border border-input bg-background hover:bg-accent hover:text-accent-foreground"
                  asChild
                >
      ...
      <div className="flex flex-col gap-4 my-5 mx-3">
        ...
        {data?.user && (
          <SheetClose asChild>
            <Link href="/bookings" className="flex gap-2 items-center">
