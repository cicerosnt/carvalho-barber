...
export function generateDayTimeList(date: Date, userId: String): string[] {
    ...
    const blockedTimesL = ["13:00", "14:00"]; 
    ...
    while (nextAvailableTime < endTime) {
        if (isAfter(nextAvailableTime, currentTime)) {
            ...
            if ((userId === "6b32011b-d357-499b-ad4d-cf742ce4072e") && !blockedTimesP.includes(formattedTime)) { //pedro
                timeList.push(formattedTime);  
            }else if (userId == "67f2bf67-28f9-4685-99fe-624c637efc45" && !blockedTimesL.includes(formattedTime)) { //leo
                timeList.push(formattedTime);  
            }else{
                timeList.push(formattedTime);  
            }
